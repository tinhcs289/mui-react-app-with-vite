...
>() {
  return {
    ...
    useInitPaginatedListState: useInitState as UseInitState<
      PaginatedListStates<Item, Filter, ExtendedStates>
    >,
    usePaginatedListAction: useAction as UseAction<
      PaginatedListStates<Item, Filter, ExtendedStates>
    >,
  };
}
...
/**
 * @example
  import PaginatedList, { ListStatesInitializer, FetchDataInitializer, ReactQueryInitializer, createStateHooks } from "@/components/list/PaginatedList;
  ...
  function SomewhereInsideMyList({ someDefaultValue }: any) {
    useInitPaginatedListState('someState', someDefaultValue, { when: 'whenever-value-changes' });

    const setState = useSetPaginatedListState();
    const someState = useGetPaginatedListState(state => state?.someState);
